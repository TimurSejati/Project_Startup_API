...
func main() {
	...
	campaignWebHandler := webHandler.NewCampaignService(campaignService, userService)
	...
	router.Use(sessions.Sessions("Cadenza97256_", cookieStore))
	...
	api.GET("/campaigns/:id", campaignHandler.GetChampaign)
	...
	router.GET("/user/edit/:id", authAdminMiddleware(), userWebHandler.Edit)
	router.POST("/user/update/:id", authAdminMiddleware(), userWebHandler.Update)
	router.GET("/user/avatar/:id", authAdminMiddleware(), userWebHandler.NewAvatar)
	router.POST("/user/avatar/:id", authAdminMiddleware(), userWebHandler.CreateAvatar)
